...
- **Keyway app <-> Chromium native host**: `KeywayChromiumBridgeIPC` carries bounded snapshot, command, focus, and result payloads over a private Unix-domain socket. Both peers authenticate the kernel-provided audit token against the canonical Apple team and code identifier before accepting data. The app owns profile/connection state; the native host owns framing, browser identity, and one connection generation.
