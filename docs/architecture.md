...
`MediaTransportTrigger` (in `MediaTransportRouteSource.swift`) is the catalogue of transport entry points—media key (keyboard and generated BetterTouchTool keys), system remote (AirPods/headset buttons, Control Center, macOS's headphone-disconnect pause), app automation (e.g. Superwhisper), explicit chooser—and documents where each enters. MediaRemote input is classified only by `init(mediaRemoteSenderID:)`: senders that are `perl` (the Apple-signed binary third-party apps use through `mediaremote-adapter` since macOS 15.4, which exits before its parent app can be resolved) or non-Apple apps other than Keyway are app automation. `routeFromCache` switches over the trigger once: a system-remote Pause is sent silently to every known target and cancels a pending app resume; an app Pause is sent the same way and remembers which targets were playing; the next app Play silently resumes only those, while an app Play with no pending app pause (e.g. a BetterTouchTool Play action) routes like any other command. Everything else uses the ordinary Target Selection Policy.

`GlobalShortcutController` registers the Settings → Shortcuts bindings (open playing source, show source chooser, send Spotify to headphones, open Keyway menu; all unset by default) as Carbon global hotkeys under its own signature, stored in UserDefaults as `shortcuts.<action>`. It re-registers when those defaults change and releases every hotkey while the Settings recorder captures a key press. Carbon hotkey handlers share the application event target, so each handler passes on hotkeys whose signature it did not register.
